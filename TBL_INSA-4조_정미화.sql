...
SELECT NAME "이름", SSN "주민번호"
     , EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(SSN, 1, 2) + 1900) + 1 "나이"
FROM TBL_INSA;
--==>>
/*
...
SELECT NAME
FROM TBL_INSA
WHERE TRUNC(EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(SSN, 1, 2) + 1900) + 1, -1) = 20;
--==>> (결과 없음)
...
SELECT NAME "이름", SSN "주민번호"
FROM TBL_INSA
WHERE EXTRACT(MONTH FROM TO_DATE(TO_CHAR('19' || SUBSTR(SSN, 1, 2)) ||
                                '-' || SUBSTR(SSN, 3, 2) || 
                                '-' || SUBSTR(SSN, 5, 2), 'YYYY-MM-DD')) = 5;
--==>>
/*
이기자	780505-2978541
장인철	780506-1625148
...
SELECT T.구분, SUM(T.인원수)
FROM 
(
    SELECT CASE WHEN GROUPING(COMMISSION_PCT) = 0
...
SELECT T.생년, COUNT(*) "직원수"
FROM
(
    SELECT SUBSTR(SSN, 1, 2)+1900 "생년"
    FROM TBL_INSA
) T
GROUP BY T.생년
ORDER BY T.생년;
...
ORDER BY EXTRACT(MONTH FROM IBSADATE)
       , EXTRACT(YEAR FROM IBSADATE) DESC;
--==>>
/*
1044	지수환	820305-1475286	2004-01-21	서울  	011-5555-7548	영업부	사원	    1060000	220000
1031	지재환	771115-1687988	2001-01-21	서울  	019-5552-7511	기획부	부장	    2450000	160000
...
SELECT DECODE(T.성별, '남자', '남자 인원수', '여자', '여자 인원수', '전체 인원수') "성별"
     , COUNT(*) "인원수"
FROM
(
    SELECT CASE WHEN SUBSTR(SSN, 8, 1) IN('1', '3') THEN '남자'
...
FROM 
(
    SELECT DECODE(SUBSTR(SSN, 8, 1), '1', '남자', '2', '여자', '알 수 없음') "성별"
         , BASICPAY "기본급"
    FROM TBL_INSA
) T
GROUP BY T.성별;
...
GROUP BY GROUPING SETS((T.부서, T.성별));
--==>>
/*
영업부	여자  	8
홍보부	남자  	3
...
GROUP BY GROUPING SETS((T.지역, T.성별));
--==>>
/*
전남  	여자  	2
전남	    남자  	1
...
GROUP BY GROUPING SETS((T.입사년도, T.성별));
--==>>
/*
2001    	여자	    2
1998	남자	    3
...
SELECT T.부서, T.성별, SUM(급여) "급여합"
FROM
(
    SELECT BUSEO "부서", BASICPAY+SUDANG "급여"
         , CASE WHEN SUBSTR(SSN, 8, 1) IN('1', '3') THEN '남자'
                WHEN SUBSTR(SSN, 8, 1) IN('2', '4') THEN '여자'
                ELSE '알 수 없음'
           END "성별"
...
, (
    ...
    GROUP BY BUSEO
    HAVING COUNT(*) >= 5
) T2 -- 여자가 5명인 부서 : 개발부, 영업부
WHERE T1.부서 = T2. 부서
GROUP BY GROUPING SETS((T1.부서, T1.성별));
--==>>
/*
영업부	여자	    8
개발부	남자	    6
...
SELECT NAME "이름"
     , DECODE(SUBSTR(SSN, 8, 1), '1', '남자', '2', '여자',
              DECODE(SUBSTR(SSN, 8, 1), '3', '남자', '4', '여자', '알 수 없음')) "성별"
...
SELECT LTRIM(TO_CHAR(IBSADATE, 'MM'), '0') || '월' "월"
     , COUNT (*) || '명' "인원수"
FROM TBL_INSA
...
SELECT NAME "이름"
     , TO_CHAR(SUBSTR(SSN, 1, 2) + 1900) || '-' || SUBSTR(SSN, 3, 2) || '-' || SUBSTR(SSN, 5, 2) "생년월일"
...
SELECT BUSEO, JIKWI, COUNT(*), SUM(BASICPAY+SUDANG), TRUNC(AVG(BASICPAY+SUDANG))
FROM TBL_INSA
GROUP BY BUSEO, JIKWI;
--==>>
/*
...
--70. 입사년도별 인원수 조회.
SELECT EXTRACT(YEAR FROM IBSADATE), COUNT(*)
FROM TBL_INSA
...
SELECT (SELECT T.인원
        FROM 
        (
            SELECT NVL(TO_CHAR(EXTRACT(YEAR FROM IBSADATE)), '전체') "입사년도"
                 , COUNT(*) "인원"
            FROM TBL_INSA
...
WHERE T.입사년도 IN ('전체', '2000', '1999', '1998');
--==>>
/*
1998	6
1999	10
...
SELECT (SELECT T.인원
        FROM
        (
            SELECT NVL(CITY, '전체') "지역", COUNT(*) "인원"
            FROM TBL_INSA
...
FROM
(
    SELECT NAME, BASICPAY, RANK() OVER(ORDER BY BASICPAY DESC) "등수"
    FROM TBL_INSA I
) T1
, (
    SELECT COUNT(*) * 0.1 "상위"
...
FROM
(
    SELECT I.*, RANK() OVER(ORDER BY BASICPAY DESC) "등수"
    FROM TBL_INSA I
) T
WHERE T.등수 <= 5;
...
FROM
(
    SELECT I.*, RANK() OVER(ORDER BY IBSADATE) "입사순"
    FROM TBL_INSA I
) T
WHERE T.입사순 <= 5;
--==>>
...
WHERE T1.BASICPAY+T1.SUDANG > T2.평균;
--==>>
/*
1001	홍길동	771212-1022432	1998-10-11	서울  	011-2356-4528	기획부	부장	    2610000	200000
1003	이순애	770922-2312547	1999-02-25	인천	010-4231-1236	개발부	부장	    2550000	160000
...
WHERE T2.급여 < T1.BASICPAY+T1.SUDANG;
--==>>
/*
1001	홍길동	771212-1022432	1998-10-11	서울	011-2356-4528	기획부	부장	2610000	200000
1046	허경운	760105-1458752	1999-05-04	경남	017-3333-3333	총무부	부장	2650000	150000
...
FROM TBL_INSA T1
, (
    SELECT AVG(SUDANG) "수당"
    FROM TBL_INSA
    WHERE BUSEO = '총무부'
) T2
WHERE T1.SUDANG > T2.수당;
--==>>
/*
1001	홍길동	771212-1022432	1998-10-11	서울  	011-2356-4528	기획부	부장	    2610000	200000
1002	    이순신	801007-1544236	2000-11-29	경기	010-4758-6532	총무부	사원	    1320000	200000
...
FROM TBL_INSA T1
, (
    SELECT BUSEO, MAX(BASICPAY) "최고"
    FROM TBL_INSA
    GROUP BY BUSEO
) T2 --부서별 가장 높은 기본급이 들어 있는 테이블
WHERE T1.BUSEO = T2.BUSEO
  AND T1.BASICPAY = T2.최고;
...
SELECT NAME, BASICPAY
     , CASE WHEN SUBSTR(T1.SSN, 8, 1) IN ('1', '3') THEN '남자'
            WHEN SUBSTR(T1.SSN, 8, 1) IN ('2', '4') THEN '여자'
            ELSE '알수없음'
       END "성별"
     , (SELECT COUNT(*) + 1
        FROM TBL_INSA T2
        WHERE T2.BASICPAY > T1.BASICPAY
          AND SUBSTR(T2.SSN, 8, 1) = SUBSTR(T1.SSN, 8, 1)) "순위"
FROM TBL_INSA T1;
--==>>
/*
홍길동	2610000	남자  	2
...
WHERE T1.BASICPAY + T1.SUDANG = T2.급여;
--==>>
/*
서울	    홍길동
인천	이순애
...
FROM
(   
    SELECT MAX(AVG(BASICPAY + SUDANG)) "최대"
    FROM TBL_INSA T
    GROUP BY CITY
) T1,
(
    SELECT CITY, AVG(BASICPAY + SUDANG) "평균"
    FROM TBL_INSA
    GROUP BY CITY
) T2
WHERE T1.최대 = T2.평균;
...
SELECT T.CITY, RANK() OVER(ORDER BY T.인원 DESC) "순위"
FROM
(
    SELECT CITY, COUNT(*) "인원"
    FROM TBL_INSA
...
FROM 
(
    SELECT TBL_INSA.NAME, BASICPAY+SUDANG "급여"
    FROM TBL_INSA
) T1
, (
    SELECT MAX(BASICPAY+SUDANG) "최대"
    FROM TBL_INSA
) T2
WHERE T1.급여 = T2.최대;
