...
--    기본적으로(default)으로 사용할 수 없는 상태로 설정되어 있으므로
--    추가 설치 및 설정 과정이 필요하다.
--    이를 위해
...
--    (다른 타입은 지원하지 않기 때문에 NUMBER 인 경우에는 TO_CHAR() 이용)
...
--		·LONG LOW 는 LONG 과 같은 제한사항을 갖는다.


-- ○ 해당 패키지를 사용할 수 있도록 권한 부여
GRANT EXECUTE ON DBMS_OBFUSCATION_TOOLKIT TO SCOTT;
--==>> Grant을(를) 성공했습니다.
--> SCOTT 계정으로 DBMS_OBFUSCATION_TOOLKIT 패키지의 프로시저를 사용 권한 부여

GRANT EXECUTE ON DBMS_OBFUSCATION_TOOLKIT TO PUBLIC;
--==>> Grant을(를) 성공했습니다.
--> PUBLIC 으로 DBMS_OBFUSCATION_TOOLKIT 패키지의 프로시저를 사용 권한 부여
