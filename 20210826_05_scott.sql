...
/*====================================================
  ...
  WHERE 조건절             -- ② : 조건
  GROUP BY 절              -- ③ : 그룹으로 묶고
  HAVING 조건절            -- ④ : 그룹에 관한 조건을 확인
  ORDER BY 절              -- ⑥ : 정렬
  
====================================================*/

--○ 현재 접속된 오라클 사용자(SCOTT) 소유의
--   테이블(TABLE), 뷰(VIEW)의 목록 조회
SELECT *
FROM TAB;
--==>>
/*
...
EMP	            TABLE	→ EMPLOYESS(사원) 테이블
SALGRADE	        TABLE	→ SAL(급여)의 등급 테이블
TBL_EXAMPLE1	    TABLE	
TBL_EXAMPLE2	    TABLE	
*/
...
--   필수 입력 사항은 ID, PW, 성명, 주민번호, 전화번호, 주소, ...
...
2. 문자형 CHAR          - 고정형 크기
          ...
          VCHAR         - 가변형 크기
          VCHAR2(10) ← '강의실'        6Byte
          VCHAR2(10) ← '잠깬서승균'    10Byte
          VCHAR2(10) ← 'OH잠깬서승균'  10Byte 를 초과하므로 입력 불가
