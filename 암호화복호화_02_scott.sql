...
IS
	...
	FUNCTION ENCRYPT(STR VARCHAR2, HASH VARCHAR2)			-- 암호화 해야 하는 대상, 키값 (암호화)
	RETURN VARCHAR2
	IS 
		PIECES_OF_EIGHT NUMBER := ((FLOOR(LENGTH(STR)/8 + .9)) * 8);	
	BEGIN
		DBMS_OBFUSCATION_TOOLKIT.DESENCRYPT
		( INPUT_STRING		=> RPAD(STR, PIECES_OF_EIGHT)	-- 변환할 값
		, KEY_STRING		=> RPAD(HASH, 8, '#')			-- 변환 패턴
		, ENCRYPTED_STRING	=> CRYPTED_STRING				-- 변환된 값
		);
		RETURN CRYPTED_STRING;	
	END;
	
	FUNCTION DECRYPT(XCRYPT VARCHAR2, HASH VARCHAR2)		-- 암호화 된 대상, 키값	(복호화)
	RETURN VARCHAR2
	IS
	BEGIN
