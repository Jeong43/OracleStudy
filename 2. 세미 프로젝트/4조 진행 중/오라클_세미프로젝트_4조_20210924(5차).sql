...
IS
 ...
 V_C_START          COURSE.C_START%TYPE;
 V_C_END            DATE;
 DUPLICATE_SUB      EXCEPTION;
 DATE_ERROR         EXCEPTION;
BEGIN
    -- 1. 과정시작일과 개강일
    -- 과목 개강일은 과정 시작일보다 빠를 수 없다.
    /*
    SELECT S_START, S_END INTO V_S_START, V_S_END
    FROM SUBJECTS
    WHERE SUB_ID = V_SUB_ID;
    */
    ...
    IF (V_S_START < V_C_START OR V_S_END > V_C_END)
        THEN RAISE DATE_ERROR;
    END IF;
    
    ------------------이 부분 수정이 필요합니다-----------------------------------------------------
    INSERT INTO SUBJECTS(SUB_NAME, S_START, S_END, BOOK_NAME)
    VALUES(V_SUB_NAME, V_S_START, V_S_END, V_BOOK_NAME);
    
    --INSERT INTO ESTABLISHED_SUB(SUB_ID,COURSE_ID,SUB_ID,ATTEND_PER,PRACTICAL_PER,WRITING_PER)
    --VALUES ('SUB' || SEQ_SUB_ID.NEXTVAL,V_COURSE_ID,V_SUB_ID,V_ATTEND_PER,V_PRACTICAL_PER,V_WRITING_PER);
    ...
    EXCEPTION
        WHEN DUPLICATE_SUB
             THEN RAISE_APPLICATION_ERROR(-20008, '이미 등록된 과목입니다.');
             ROLLBACK;
        WHEN DATE_ERROR
            THEN RAISE_APPLICATION_ERROR(-20009, '입력정보 오류');
                 ROLLBACK;
        WHEN OTHERS
             THEN ROLLBACK;
